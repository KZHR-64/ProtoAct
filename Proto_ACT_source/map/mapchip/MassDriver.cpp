#pragma once
#include "DxLib.h"
#include "MassDriver.h"

using namespace std;

//コンストラクタ
MassDriver::MassDriver(MapChipManager* man, int num) : AbstractMapChip(man, num) {
	loaded = false;
	fire_time = 0;
	hit_flag = false;
}
...
//必ず行う更新
void MassDriver::AbsUpdate() {

}

//更新
void MassDriver::Update(shared_ptr<Player> player, shared_ptr<EnemyManager> enemy, shared_ptr<BulletManager> bullet, shared_ptr<EffectManager> effect, shared_ptr<Key> key, shared_ptr<Sound> sound) {
	//自機の座標を取得
...
//描画
void MassDriver::Draw(shared_ptr<Image> image, shared_ptr<Camera> camera) {
